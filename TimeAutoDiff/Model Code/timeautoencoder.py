...
def compute_sine_cosine(v, num_terms):
    "Authors use a cyclic pattern to turn categorical values to numeric"
...
class DeapStack(nn.Module):
    def __init__(self, channels, batch_size, seq_len, n_bins, n_cats, n_nums, cards,
                 input_size, hidden_size, num_layers, cat_emb_dim, time_dim, lat_dim):
        super().__init__()
...
def auto_loss(inputs, reconstruction, n_bins, n_nums, n_cats, beta, cards):
    """ Calculating the loss for denoising autoencoder (DAE) network.
    ...
    discrete_loss = 0 # for binary + categorical variables
    numeric_loss  = 0 # for numerical/continuous variables

    if 'bins' in reconstruction:
        discrete_loss += F.binary_cross_entropy_with_logits(reconstruction['bins'], bins)
    ...
    if 'cats' in reconstruction:
        ...
        discrete_loss += torch.stack(cats_losses).mean()

    if 'nums' in reconstruction:
        numeric_loss = F.mse_loss(reconstruction['nums'], nums)

    return discrete_loss, numeric_loss
...
def train_autoencoder(real_df, processed_data, channels, hidden_size, num_layers, lr, weight_decay, n_epochs,
    ...
    """Outputs many things, among them the loss, split into 2:
        1. KL divergence loss
        2. reconstruction loss (split into numeric and discrete loss)"""
    ...
    ae     = DeapStack(channels, batch_size, seq_len, n_bins, n_cats, n_nums, cards, input_size,
                       hidden_size, num_layers, emb_dim, time_dim, lat_dim).to(device)
    ...
    with Progress() as progress:
        ...
        for epoch in range(n_epochs):
            ...
            kl_divergence_loss = -0.5 * torch.mean(temp.mean(-1).mean())
            discrete_loss, numeric_loss = auto_loss(inputs[batch_indices,:,:], outputs, n_bins, n_nums, n_cats, beta, cards)
            
            # loss_Auto = numeric_loss + discrete_loss + beta * kl_divergence_loss
            loss_Auto = numeric_loss + discrete_loss # removed KL loss, AE dont care about it but VAE do
            loss_Auto.backward()
            optimizer_ae.step()
            ...
            if loss_Auto < best_train_loss:
                ...
                patience += 1
                if patience == patience_crit:
                    if beta > min_beta:
                        beta = beta * beta_decay_factor
            
            reconst_loss.append(numeric_loss.item() + discrete_loss.item())
            KL_loss.append(kl_divergence_loss.item())
    
    output, latent_features, _, _ = ae(processed_data)
        
    #return (ae, latent_features.detach(), output, losses, reconst_loss, KL_loss)
    # return (ae, latent_features.detach(), output, losses, reconst_loss, mu_z, logvar_z)
    return (ae, latent_features.detach(), output, losses, reconst_loss, mu_z, logvar_z)
